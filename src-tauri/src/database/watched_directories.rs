...
mod tests {
    ...
    #[test]
    fn test_create_watched_directory() {
        let conn = get_test_connection();
        let dir = WatchedDirectory::new("/path/to/photos".to_string(), true);

        let id = create(&conn, &dir).unwrap();
        assert!(id > 0);

        // Test duplicate path
        let result = create(&conn, &dir);
    ...
    fn test_find_by_id() {
        ...
        assert_eq!(found.unwrap().directory_path, "/path/to/photos");

        let not_found = find_by_id(&conn, 9999).unwrap();
    ...
    fn test_update_status() {
        ...
        update_status(&conn, id, "paused", None).unwrap();
        let updated = find_by_id(&conn, id).unwrap().unwrap();
        assert_eq!(updated.status, "paused");

        // Test error status requires message
        let result = update_status(&conn, id, "error", None);
        assert!(result.is_err());

        update_status(&conn, id, "error", Some("Test error".to_string())).unwrap();
        let updated = find_by_id(&conn, id).unwrap().unwrap();
    ...
    fn test_get_stats() {
        ...
        let stats = get_stats(&conn, id).unwrap();
        assert!(stats.is_some());
        let stats = stats.unwrap();
        assert_eq!(stats.directory_path, "/path/to/photos");
        assert_eq!(stats.active_photos, 0);
